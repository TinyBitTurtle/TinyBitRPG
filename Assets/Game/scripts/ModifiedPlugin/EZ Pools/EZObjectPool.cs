...
{
    [AddComponentMenu("EZ Object Pool/My Object Pool")]
    public class EZObjectPool : MonoBehaviour
    {
        ...
        public virtual void InstantiatePool()
        {
            ...
            for (int i = 0; i < PoolSize; i++)
            {
                ...
                {
                    AccProbability += Templates[j].probability;
            ...
            for (int j = 0; j < Templates.Length; ++j)
            {
                Templates[j].Template.SetActive(false); ;
            }
        }
        ...
        public virtual bool TryGetNextObject(Vector3 pos, Quaternion rot, out GameObject obj)
        {
        ...
        protected GameObject  NewActiveObject()
        {
            // chose according to each prob probability a template
            int idx = Random.Range(1, 101);
            float AccProbability = 0.0f;
